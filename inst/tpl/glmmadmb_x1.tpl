...
DATA_SECTION
  ...
  init_int zi_model_flag                        // non-trivial ZI model?
...
PARAMETER_SECTION
 LOC_CALCS
  ...
  // Determines "phases", i.e. when the various parameters  become active in the optimization process
  ...
  int rand_phase = no_rand_flag==0 ? pctr++ : -1;    				// SD of REs
  int cor_phase = (rand_phase>0) && (sum(cor_flag)>0) ? pctr++ : -1 ; 		// Correlations of REs
